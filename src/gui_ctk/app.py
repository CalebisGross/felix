...
class FelixApp(ctk.CTk):
    ...
    def _create_status_bar(self):
        ...
        self.version_label = ctk.CTkLabel(
            self.status_bar,
            text="Felix v0.9.0 (CTk Edition)",
            font=ctk.CTkFont(size=11),
            text_color=get_theme_manager().get_color("fg_muted"),
            anchor="e"
        )
        self.version_label.grid(row=0, column=1, sticky="e", padx=10, pady=5)
    ...
    def _show_error(self, title: str, message: str):
        ...
        # CTkInputDialog doesn't have a message-only mode, so we use a simple approach
    ...
    def _on_close(self):
        ...
        if self.system_running:
            logger.info("Stopping system before exit...")
            if self.felix_system:
                self.felix_system.stop()
